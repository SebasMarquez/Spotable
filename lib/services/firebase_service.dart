...
class FirebaseService {
  ...
  static Future<String> createOrder(String idCantina, Map<String, dynamic> orderData) async {
    ...
    print('📍 Ruta completa: $_restauranteCollection/$idCantina/$_ordersSubCollection');
    ...
    try {
      ...
      print('🔗 Ruta completa: $_restauranteCollection/$idCantina/$_ordersSubCollection/${docRef.id}');
      print('=======================================');
      ...
      print('   Mensaje: $e');
      print('   Restaurante ID: $idCantina');
      print('   Ruta intentada: $_restauranteCollection/$idCantina/$_ordersSubCollection');
      print('=======================================');
      throw e;
    }
  }
  ...
  static Future<void> debugFirebaseStructure(String idCantina) async {
    ...
    try {
      ...
      final restaurantesSnapshot = await _firestore
          .collection(_restauranteCollection)
          .limit(5)
          .get();
      ...
      if (restaurantDoc.exists) {
        ...
        final commonNames = ['orders', 'ordenes', 'Orders', 'Ordenes', 'pedidos', 'Pedidos'];
        for (String name in commonNames) {
          try {
            ...
            if (snapshot.docs.isNotEmpty) {
              print('   ✅ Subcolección "$name" existe y tiene ${snapshot.docs.length} documento(s)');
            }
          } catch (e) {
            // Silently ignore
          }
  ...
  static Stream<List<MenuItem>> getMenuItemsByCategory(String id_cantina, String categoria) {
    return _firestore
        .collection(_restauranteCollection)
        .doc(id_cantina)
        .collection(_menuSubCollection)
